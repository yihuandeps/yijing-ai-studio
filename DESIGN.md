...
**核心区块**：① 对话创作流 + 图片上传 ② 商家资产库 ③ 商家场景库 ④ 公共场景库 ⑤ 风格提示词库
...
- 素材面板四个页签：**资产库 / 我的场景 / 公共场景 / 风格库**，可整体收起；风格库为纯文本卡片列表（风格名+推荐画幅+定位+两枚插入按钮），点击将专业提示词写入输入框并自动选中【服装描述】占位；提示词内画幅与模特段均为动态占位：画幅按所选比例填充、模特按模特选择器填充（跟随风格推荐 / 预设数字模特 / 自定义弹层），切换后实时同步输入框文本；清晰度（1K/2K/4K）为独立生成参数、不写入提示词文本；模特选择器 popover 带 26px 圆形头像，含「年龄×长相」组合 chip 区（复用 .tag 样式，单选可反选，未选维度跟随风格推荐），自定义弹层为居中模态卡（遮罩 rgba(text,.28)+blur(4px)，主按钮用 --grad）
